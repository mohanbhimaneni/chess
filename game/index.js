...
function move(element) {        //This function is called in index.html
    console.log("clicked");
    if(element.classList[0]!='boardbg' &&element.classList[1][0]==chance[0] && clicked==0){
...
async function movePiece(element){
    ...
    addMoves(element);
    removeAllSelected();
    if(new1.classList[2]=='p'){
        if(new1.classList[1]=='wh')
        moveCount['whp'][new1.classList[4]]+=1;
        else
    ...
    console.log(chance,masterCount);
}
